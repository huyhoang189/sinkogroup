...
// Thêm action để hiển thị xem trước file
add_action('admin_enqueue_scripts', 'custom_media_preview');

function custom_media_preview() {
    // Chỉ thêm xem trước trong trang thêm hoặc sửa bài viết
    global $pagenow;
    if ('post.php' !== $pagenow && 'post-new.php' !== $pagenow) {
        return;
    }

    // Thêm script và style
    wp_enqueue_script('media-preview', get_template_directory_uri() . '/media-preview.js', array('jquery'), null, true);
    wp_enqueue_style('media-preview-style', get_template_directory_uri() . '/css/media-preview.css');
}

// Thêm action để lấy thông tin về file
add_action('wp_ajax_get_media_info', 'get_media_info_callback');

function get_media_info_callback() {
    // Kiểm tra quyền truy cập
    if (!current_user_can('edit_posts')) {
        wp_send_json_error('Bạn không có quyền truy cập.');
    }

    // Lấy ID của file từ request
...
// Trong file functions.php của theme hoặc plugin của bạn
